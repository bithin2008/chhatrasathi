...
export class AllQuestionsComponent implements OnInit {
  ...
  getMyQuestionList() {
    ...
    let url = "questions" + '?class=' + localStorage.getItem('classId') + '&subject=' + this.subject + '&chapter=' + this.chapter + '&page=' + this.page + '&pageSize=' + this.pageSize + '&lang=' + localStorage.getItem('language') + "&token=" + localStorage.getItem('token');
    this._commonService.get(url).subscribe((response) => {
      ...
      if (response.status == 1) {
        if (response.results.length > 0) {
          ...
          this.allQuestionList = this.allQuestionList.concat(response.results);
        }
      } else if (response.status == 401) {
        this.showToast('error', this.messsageObj.allQuestion.invalidMessage[this.defaultLanguage], response.message, 3000, '/login')
      } else {
        this.showToast('error', this.messsageObj.allQuestion.errorMessage[this.defaultLanguage], response.message, 2500, '')
      }
    }, (error) => {
      this.enableLoader = false;
      ...
      buttons: [
        {
          ...
          handler: () => {
            ...
            this._commonService.delete(url).subscribe((response) => {
              ...
              this.enableLoader = false;
              if (response.status == 1) {
                this.showToast('success', this.messsageObj.allQuestion.deleteMessage[this.defaultLanguage], response.message, 3500, '/ask-question/my-questions')
                this.allQuestionList.forEach((element, indx) => {
                  if (element._id == obj._id) {
                    this.allQuestionList.splice(indx, 1)
                  }
                });
              } else if (response.status == 401) {
                this.showToast('error', this.messsageObj.allQuestion.invalidMessage[this.defaultLanguage], response.message, 3000, '/login')
